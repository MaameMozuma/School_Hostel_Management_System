                ...
                <table id="table1" >
                    ...
                    <tbody>
                        <?php
                          ...
                          $abdi_sql = "SELECT room.room_name, room.capacity, room.capacity - COUNT(DISTINCT booking.student_id) 
                                       AS available_space, 
                                       CASE WHEN room.capacity = (room.capacity - COUNT(DISTINCT booking.student_id)) 
                                       THEN 'Empty'
                                       ...
                                       AND booking.year_id = (SELECT year_id FROM academic_year 
                                       WHERE a_year = '2022/2023')
                                       AND booking.term_id = (SELECT term_id FROM school_term 
                                       WHERE term = 'Term Two')
                                       WHERE room.room_name LIKE 'a%'
                                       GROUP BY room.room_id
                                       ORDER BY CAST(SUBSTRING(room.room_name, 2) AS UNSIGNED);";
